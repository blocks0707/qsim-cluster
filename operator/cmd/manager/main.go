...
func init() {
	...
	utilruntime.Must(quantumv1alpha1.AddToScheme(scheme))
...
func main() {
	...
	// Setup QuantumJob controller
	if err = (&controller.QuantumJobReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "QuantumJob")
		os.Exit(1)
	}
	...
	if err = (&controller.QuantumNodeProfileReconciler{
		...
		setupLog.Error(err, "unable to create controller", "controller", "QuantumNodeProfile")
		os.Exit(1)
	}
...
`
	fmt.Print(banner)
	setupLog.Info("Quantum Operator started successfully")
	setupLog.Info("Watching for QuantumJob and QuantumNodeProfile resources")
}